...
namespace Domain
{
	...
	public class Order : EntityBase, IValidable
	{
        ...
        public virtual IEnumerable<OrderItem> Items { get { return m_items; } }

        public virtual decimal Total { get { return Items.Sum(a_i => a_i.Quantity*a_i.UnitPrice); } }
        
        public virtual IEnumerable<string> GetErrorMessages()
	    {
            if (!Items.Any()) yield return "The order must include at least one item.";

            var itemsErrors = Items.SelectMany((a_i, a_index) => a_i.GetErrorMessages()
                                                              .Select(a_m => string.Format("Item {0}: {1}", a_index, a_m)));
